...
class DataPrepration(object):
    ...
    def fetch_groups(self):
        ...
        with open("OutPuts/groups.csv", 'w') as file:
        ...
        for organizer in my_cursor:
            ...
            if 14912 >= organizer[0] >= 14639 or 20798 >= organizer[0] >= 20734:
                ...
                for members in my_cursor:
                    group_set.add(members[0])
                groups_list.append(group_set)
        with open("OutPuts/groups_members.csv", 'w') as file:
            writer = csv.writer(file)
    ...
    def get_individual_ratings(self):
        with open("OutPuts/individual_ratings.csv", 'w') as file:
            writer = csv.writer(file)
    ...
    def get_group_ratings(self):
        with open("OutPuts/group_ratings.csv", 'w') as file:
            writer = csv.writer(file)
            writer.writerow(['user', 'restaurant','rating'])
            query = "SELECT DISTINCT(user_id) FROM  user_restaurant_rating_post"
            my_cursor = self.mysql_conn.cursor(buffered=True)
    ...
    def get_user_support(self):
        with open("OutPuts/support_type.csv", 'w') as file:
            writer = csv.writer(file)
    ...
    def group_organizer_type(self):
        with open("OutPuts/group_organizer_type.csv", 'w') as file:
            writer = csv.writer(file)
    ...
    def get_organizer_visited_rest(self):
        with open("OutPuts/organizer_visiting.csv", 'w') as file:
            writer = csv.writer(file)
    ...
    def get_organizer_booked_rest(self):
        with open("OutPuts/organizer_booked.csv", 'w') as file:
            writer = csv.writer(file)
            writer.writerow(['user_id', 'group_id','rest_id','avialble_rest','sort_pop','sort_fit','sort_sim','fit_score','sim_score','c1_fit','c2_fit','c3_fit','c4_fit','c5_fit','fit_rank','sim_rank','pop_rank','tip','category','dish','price','time'])
            query = "SELECT DISTINCT(user_id) FROM  user_group_info"
            my_cursor = self.mysql_conn.cursor(buffered=True)
    ...
    def get_organizer_used_sorting(self):
        with open("OutPuts/organizer_sorting.csv", 'w') as file:
            writer = csv.writer(file)
    ...
    def get_organizer_used_explanasion(self):
        with open("OutPuts/organizer_explanasions.csv", 'w') as file:
            writer = csv.writer(file)
    ...
    def get_organizer_conflict(self):
        with open("OutPuts/organizer_conflict.csv", 'w') as file:
            writer = csv.writer(file)
    ...
    def get_organizer_usage_conflict(self):
        with open("OutPuts/organizer_usage_conflict.csv", 'w') as file:
            writer = csv.writer(file)
    ...
    def get_organizer_remember(self):
        with open("OutPuts/organizer_remeber.csv", 'w') as file:
            writer = csv.writer(file)
    ...
    def get_organizer_usage_remember(self):
        with open("OutPuts/organizer_usage_remeber.csv", 'w') as file:
            writer = csv.writer(file)
            ...
            for user in my_cursor:
                if 14912 >= user[0] >= 14639  or 20798 >= user[0] >= 20734:
                    ...
                    my_cursor.execute(query)
                    for remeber in my_cursor:
                        temp_list = [user[0], remeber[0], int(remeber[1]), int(remeber[2]), int(remeber[3]), int(remeber[4]), int(remeber[5]), int(remeber[6]), int(remeber[7]), int(remeber[8]), int(remeber[9]), int(remeber[10]), int(remeber[11]), int(remeber[12]), int(remeber[13]), int(remeber[14]), remeber[15], remeber[16], remeber[17], remeber[18]]
                        writer.writerow(temp_list)
            file.close()
            
    def extrac_preference_insertion(self):
         with open("OutPuts/preference_insertion_table.csv", 'w') as file:
            writer = csv.writer(file)
            ...
            for row in my_cursor:
                ...
                if (interaction_string_list[0] == "Preference"):
                    if str(interaction_string_list[-1]) != 'undefined' and (20734 >= int(interaction_string_list[-1]) >= 14639 ):
                        temp_list.append(interaction_string_list[-1])
                        temp_list.append(row[0])
                        temp_list.append(interaction_string_list[1])
                        temp_list.append(row[2].strftime("%m/%d/%Y, %H:%M:%S"))
                        temp_list.append(0)
                        writer.writerow(temp_list)
            file.close()
            df_preferenc = pd.read_csv("OutPuts/preference_insertion_table.csv")
            ...
            for duplicated_rows in df_preferenc[df_preferenc[['user_id', 'group_id','preference']].duplicated(keep='last')].to_dict(orient="records"):
                ...
                for repition in df_repition.values:
                    ...
                    indexing += 1
            df_preferenc.to_csv("OutPuts/preference_insertion_deleted_table.csv")
                             
    def update_individual_scores(self):
        df_preferenc = pd.read_csv("OutPuts/organizer_visiting.csv").reindex()
        ...
        df_preferenc.to_csv("OutPuts/organizer_visiting_cleaned.csv")
        print("First Done")
        df_preferenc = pd.read_csv("OutPuts/organizer_booked.csv").reindex()
        for index, row in df_preferenc.iterrows():
            print(index)
            max_member_size = 5
            group_id = row['group_id']
            ...
            if counter > 0:
                df_preferenc.iloc[index, df_preferenc.columns.get_loc('fit_score')] = scores_sum / counter
            df_preferenc.iloc[index, df_preferenc.columns.get_loc('sim_score')] = self.similarity(group_id,rest_id , visiting_time)
        df_preferenc.to_csv("OutPuts/organizer_booked_cleaned.csv")
                
    def update_rest_info(self):
        df_preferenc = pd.read_csv("OutPuts/organizer_visiting_cleaned.csv").reindex()
        for index, row in df_preferenc.iterrows():
            rest_id = row['rest_id']
        ...
        df_preferenc = pd.read_csv("OutPuts/organizer_booked_cleaned.csv").reindex()
        for index, row in df_preferenc.iterrows():
            rest_id = row['rest_id']
            menu = Restaurant(self.mysql_conn).get_restaurant_menu_id(rest_id)
            del menu['id']
